...
# DEPREC
def get_data_single_date(root_dir, year, month, day, north=90, south=-90, west=-180, east=180, collection="5110",
                         dayOrNight="DNB",
                         mode="download"):
    '''
    FUNCTION DEPRECIATED - USE get_data() instead
    '''
...
def download_files_from_server(root_dir, file_urls):
    ...
    for i, file in enumerate(fpath):
        ...
        # Use wget for large files
        # headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36'}
        wget.download(file_urls[i], file)
        if not os.path.isfile(file):
            raise ConnectionError(
