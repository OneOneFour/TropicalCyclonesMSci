...
def interpolate_start():
    return {"ISO_TIME": Timestamp(datetime(year=2020, month=1, day=1, hour=12, minute=0, second=0)), "A": 0, "B": 5,
            "C": np.nan, "D": np.nan, "E": 0}


@pytest.fixture()
def interpolate_end():
    return {"ISO_TIME": Timestamp(datetime(year=2020, month=1, day=1, hour=15, minute=0, second=0)), "A": 1,
            "B": np.nan, "C": 4, "D": np.nan, "E": "E"}
