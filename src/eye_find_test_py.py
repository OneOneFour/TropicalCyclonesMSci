...
for y in range(0, hot_point_ind[0]):
    if I05array[hot_point_ind[0]-y, hot_point_ind[1]] < I05array[hot_point_ind] - threshold:
...
for x in range(0, hot_point_ind[1]):
    if I05array[hot_point_ind[0], hot_point_ind[1]-x] < I05array[hot_point_ind] - threshold:
...
fig, ax = plt.subplots()
im = ax.imshow(eye_data, cmap="jet")
fig.colorbar(im)

fig, ax = plt.subplots()
im = ax.imshow(I05array, cmap="jet")
fig.colorbar(im)
